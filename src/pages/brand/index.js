...
const Wrapper = ({children}) => {
    return (
        <Container className="items-center grid grid-cols-5 gap-4 w-full text-[14px] leading-[16px] font-bold">{children}</Container>
    )
}

const ItemsWrapper = ({children}) => {
    return (
        <Item className="items-center w-full  h-[220px] text-[14px] leading-[16px] font-bold">{children}</Item>
    )
}
