...
export const TextInputOnly = ({value, placeholder, action, className, onFocus, onBlur, autoFocus}) => {
    const [inputValue, setInputValue] = useState(value)
    return (
        <input 
            autoFocus={autoFocus} 
            ...
            className={"text-input-normal " + (className!==undefined?className:"")}
        />
    )
}
export const TextInputOnlyWithEnterKey = ({value, placeholder, action, className}) => {
    const [inputValue, setInputValue] = useState(value)
    return (
