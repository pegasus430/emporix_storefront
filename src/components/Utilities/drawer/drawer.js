...
const Drawer = ({children}) => {
    ...
    return (
        <main className={"fixed overflow-hidden z-50 bg-gray-900 bg-opacity-25 inset-0 transform ease-in-out " +
                (showCart==true? "transition-opacity opacity-100 duration-500 translate-x-0  "
            ...
            <section className=" w-screen h-full cursor-pointer " onClick={() => { setShowCart(false);}}></section>
        </main>
    )
}
